# 任务 09：question-detail 题目详情（待实现）

> 开始前请先阅读 `00_总起说明_每个任务必读.md`

## 任务目标

将 question-detail 页面从 stub 实现为完整页面。展示题干、答题结果、关联模型/知识点及来源。

## 参考资料

- 页面说明：`docs/第二阶段_html转flutter/18个html页面的分别说明文档/09_question-detail_题目详情.md`
- 页面截图：`docs/第一阶段_html细化/html截图验证/question-detail/full/question-detail__390x844__full.png`
- 组件截图：`docs/第一阶段_html细化/html截图验证/question-detail/components/`

## 目标文件

- `echomind_app/lib/features/question_detail/question_detail_page.dart`
- `echomind_app/lib/features/question_detail/widgets/top_frame_widget.dart`
- `echomind_app/lib/features/question_detail/widgets/question_content_widget.dart`
- `echomind_app/lib/features/question_detail/widgets/answer_result_widget.dart`
- `echomind_app/lib/features/question_detail/widgets/question_relations_widget.dart`
- `echomind_app/lib/features/question_detail/widgets/question_source_widget.dart`

## 组件清单

| 组件 | 功能 | 跳转 |
|------|------|------|
| top-frame | 返回与标题 | questionAggregate（返回） |
| question-content | 题干/题图内容 | 无 |
| answer-result | 对错状态 + 进入诊断入口 | aiDiagnosis |
| question-relations | 归属模型与关联知识点 | knowledgeDetail, modelDetail |
| question-source | 来源卷子、题号、分值、态度 | 无 |

## 页面结构

```
Scaffold → SafeArea → Column [TopFrame, Expanded(ListView [QuestionContent, AnswerResult, QuestionRelations, QuestionSource])]
```

## 输出要求

1. **截图留档**：保存页面效果截图到 `docs/第二阶段_html转flutter/截图验证/09_question-detail_题目详情_完成效果.png`
2. **完成说明**：创建 `docs/第二阶段_html转flutter/18个页面的完成说明文档/09_question-detail_题目详情_完成说明.md`，内容包括：实现的组件列表、与截图的差异说明、路由跳转验证结果、已知问题或遗留项
3. **拉起下一任务**：完成后自动读取 `10_ai-diagnosis_AI诊断.md` 并开始执行下一个页面的工作流

---

# 附录：页面说明文档（question-detail 题目详情）
